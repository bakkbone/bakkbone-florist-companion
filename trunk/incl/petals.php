		...
		$bkfoptions = get_option("bkf_options_setting");
			if ( $column == 'petals' ) {
				...
				if ( $order->has_status( 'new' ) ) {
					echo '<a href="https://api.bakkbone.au/webhook/' . esc_html($bkfoptions["bkf_petals_member_number"]) . '/accept?id=' . esc_html( $post_id ) . '"><img src="' . plugins_url() . '/bakkbone-florist-companion/incl/img/Accept.png" width="30px" title="Accept" /></a>&nbsp;<a href="' . site_url() . '/reject?id=' . $post_id . '"><img src="' . plugins_url() . '/bakkbone-florist-companion/incl/img/Reject.png" width="30px" title="Reject" /></a>';
			}
		}
	}
