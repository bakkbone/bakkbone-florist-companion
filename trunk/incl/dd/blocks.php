...
class BkfDdBlocks{
    ...
    function bkf_admin_menu(){
        ...
        __("Blocked Delivery Dates","bakkbone-florist-companion"),//page title
        '— ' . __("Blocked Dates","bakkbone-florist-companion"),//menu title
        "manage_options",//capability
        "bkf_ddb",//menu slug
        array($this, "bkf_dd_blocks_page"),//callback
        31
        );
    }
                ...
                <div class="inside bkf-inside" style="width:50%;">
					...
					<div class="bkf-form" style="max-width:200px;text-align:center;margin: 12px auto;">
						<form id="add-closed" action="<?php echo $ajaxurl; ?>" />
							...
							<p style="margin:5px 0;"><input type="submit" class="button button-primary button-large" value="Add Date as Closed"></p>
						</form>
					...
					<ul style="display:grid;grid-template-columns:auto auto;width:100%;"><?php foreach($closedsort as $ts => $ds){
						echo '<li class="bkf-list-group" style="display:flex;flex-direction:row;justify-content:space-between;align-items:center;"><p>' . $ds . '</p><a style="height:20px;" href="' . $ajaxurl . '?action=bkf_dd_remove_closed&ts='.$ts.'&nonce='.$rcnonce.'"><div class="bkf-close tooltip"><span class="tooltiptext">Unblock Date</span></div></a></li>';
					}; ?>
				</ul><? } ?>
                </div>
                <div class="inside bkf-inside" style="width:50%;">
					...
					<div class="bkf-form" style="max-width:200px;text-align:center;margin: 12px auto;">
						<form id="add-full" action="<?php echo $ajaxurl; ?>" />
							...
							<p style="margin:5px 0;"><input style="margin-left:0;" type="text" name="date" class="closure-date input-text bkf-form-control" required autocomplete="off" /></p>
							<p style="margin:5px 0;"><input type="submit" class="button button-primary button-large" value="Add Date as Fully Booked"></p>
						</form>
					...
					<ul style="display:grid;grid-template-columns:auto auto;width:100%;"><?php foreach($fullsort as $ts => $ds){
						echo '<li class="bkf-list-group" style="display:flex;flex-direction:row;justify-content:space-between;align-items:center;"><p>' . $ds . '</p><a style="height:20px;" href="' . $ajaxurl . '?action=bkf_dd_remove_full&ts='.$ts.'&nonce='.$rfnonce.'"><div class="bkf-close tooltip"><span class="tooltiptext">Unblock Date</span></div></a></li>';
					}; ?>
				</ul><? } ?>
                </div>
            </div>
		      ...
		      document.addEventListener('DOMContentLoaded', function() {
				...
				if(null !== $closed){
					foreach($closed as $ts => $date){
						...
						echo '{ title: \'Closed\', start: \'' . $string . '\', className: \'closedbg\' }, ';
					}
				}
				...
				if(null !== $full){
					foreach($full as $ts => $date){
						$string = date("Y-m-d",$ts);
						echo '{ title: \'Fully Booked\', start: \''.$string.'\', className: \'fullbg\' }, ';
					}
					
					if(get_option('bkf_dd_setting')['monday'] == false){
						echo '{ title: \'Closed (Global)\', daysOfWeek: \'1\', className: \'closedbg\' }, ';
					}
					if(get_option('bkf_dd_setting')['tuesday'] == false){
						echo '{ title: \'Closed (Global)\', daysOfWeek: \'2\', className: \'closedbg\' }, ';
					}
					if(get_option('bkf_dd_setting')['wednesday'] == false){
						echo '{ title: \'Closed (Global)\', daysOfWeek: \'3\', className: \'closedbg\' }, ';
					}
					if(get_option('bkf_dd_setting')['thursday'] == false){
						echo '{ title: \'Closed (Global)\', daysOfWeek: \'4\', className: \'closedbg\' }, ';
					}
					if(get_option('bkf_dd_setting')['friday'] == false){
						echo '{ title: \'Closed (Global)\', daysOfWeek: \'5\', className: \'closedbg\' }, ';
					}
					if(get_option('bkf_dd_setting')['saturday'] == false){
						echo '{ title: \'Closed (Global)\', daysOfWeek: \'6\', className: \'closedbg\' }, ';
					}
					if(get_option('bkf_dd_setting')['sunday'] == false){
						echo '{ title: \'Closed (Global)\', daysOfWeek: \'0\', className: \'closedbg\' }, ';
					}
				}
				?>
				]
	...
	{
		register_setting(
			"bkf_dd_blocks_group",// group
			"bkf_dd_closed", //setting name
			array('type' => 'array') //sanitize_callback
		);
		register_setting(
			"bkf_dd_blocks_group",// group
			"bkf_dd_full", //setting name
			array('type' => 'array') //sanitize_callback
		);
	}
	...
	function bkfDdbAdminFooter()
	{
		...
		if($screen->id == "settings_page_bkf_ddb")
		{
	...
	{
		$screen = get_current_screen();
		if($screen->id == "florist-options_page_bkf_ddb")
		{
