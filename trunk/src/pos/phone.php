...
class BkfPho{
	...
	function phone_order_content(){
						...
						<fieldset id="delivery_details" style="display:none">
							...
							<table class="form-table" id="delivery_form" style="display:none;">
								<tbody>
									...
									<tr>
										<th scope="row"><label for="delivery_notes"><?php echo DELIVERYNOTESTEXT; ?></label></th>
		...
		<tr class="delivery_timeslot_row bkf-hidden" id="delivery_timeslot_row">
			<th scope="row"><?php echo __('Timeslot', 'bakkbone-florist-companion'); ?> <abbr class="required" title="required">*</abbr></label></th>
			<td>
			<select name="delivery_timeslot" id="delivery_timeslot" class="delivery_timeslot bkf-form-control select2">
			</select>
			<div class="tsfee bkf-input-icon bkf-hidden"><input class="bkf-form-control med-text" type="number" name="timeslot_fee" id="timeslot_fee" step="0.01"><i><?php echo bkf_currency_symbol(); ?></i></div>
			</td>
									</tr>
									<tr>
										<th scope="row"><label for="card_message"><?php echo CARDMESSAGETEXT; ?></label></th>
   	    	...
   	    	<script id="bkf_ddfield">
   	 			...
   	 			<?php if(get_option('bkf_dd_setting')['monday'] == false){ ?>
   	 				...
   	 				if(isset(get_option('bkf_dm_setting')['monday'])){
   	 					...
   	 					if(w == 1 && monday_items.includes(currentShippingMethod)){
   	 						return [true, "unavailable", '<?php echo BKF_DD_UA_TEXT; ?>'];
   	 					}
   	 					<?php
   	 				}
   	 			}; ?>
   			 ...
   			 <?php if($sdcpassed == true){
   				 ...
   				 if(d == "<?php echo wp_date("j"); ?>" && y == "<?php echo wp_date("Y"); ?>" && m == <?php echo wp_date("n"); ?> - 1){
   					 return [true, "sdc","<?php echo BKF_DD_SDC_TEXT; ?>"];
   					 }<?php
   			 }?>
...
loading.fadeOut();
		...
		<script type="text/javascript" id="method_script">
			jQuery(document.body).on( 'change', 'input[name="ordertype"]', function($) {
				...
				const requiredDeliveryFields = [ jQuery('input[name="delivery[first]"]'), jQuery('input[name="delivery[last]"]'), jQuery('input[name="delivery[address_1]"]'), jQuery('input[name="delivery[phone]"]'), jQuery('input[name="delivery[city]"]'), jQuery('input[name="delivery[postcode]"]'), jQuery('input[name="delivery[state]"]'), jQuery('input[name="delivery[country]"]') ];
