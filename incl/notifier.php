...
class BkfNotifier{
	...
	function __construct()
	{
		...
		if($features['order_notifier'] == 1){
	        ...
	        add_action('wp_ajax_notifier', array($this, 'bkf_notifier_ajax') );
	        add_action('wp_ajax_notifier_status', array($this, 'bkf_notifier_status_ajax') );
		...
		<script type="text/javascript" id="order_notifier_js">
			...
			function toggleNotifierStatus(){
				...
				if (toggle.checked == true){
					...
					jQuery( function($){ $.ajax({url: toggleUrl, type: 'POST'}); });
				} else {
	...
	function bkf_notifier_help(){
		...
		<h2><?php echo BKF_HELP_SUBTITLE; ?></h2>
			<a href="https://docs.bkbn.au/v/bkf/notifier" target="_blank">https://docs.bkbn.au/v/bkf/notifier</a>
		<?php
	}
	
	function bkf_notifier_ajax(){
		...
		foreach($orders as $order){
			$result[] = array(
				...
				'value' => get_woocommerce_currency_symbol(get_woocommerce_currency()).$order->get_total(),
	...
	function bkf_notifier_status_ajax(){
