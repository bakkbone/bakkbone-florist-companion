...
class BkfOrderStatus{
  ...
  public function __construct() {
    ...
    add_filter( 'woocommerce_menu_order_count', array($this, 'bkf_woocommerce_menu_order_count'), 10, 1 );
  }
  
  function bkf_woocommerce_menu_order_count( $ordercount ) {
      $ordercountbkfnew = wc_orders_count( 'new' );
    
      return $ordercount + $ordercountbkfnew;
  }
  ...
  public function bkf_add_status( $order_statuses )
  {
    ...
    foreach ( $order_statuses as $key => $status ) {
        ...
        if ( 'wc-processing' === $key ) {
            ...
            $new_order_statuses['wc-payment-await'] = _x( 'Awaiting payment', 'Order status', 'bakkbone-florist-companion' );
        }
    }
    ...
    function bkf_include_status_to_reports($order_statuses)
    {
        if ( is_array( $order_statuses ) && in_array( 'completed', $order_statuses, true ) ) {
            $bkfoptions = get_option("bkf_features_setting");
            if($bkfoptions["petals_on"] == true) {
                return array_merge( $order_statuses, array( 'made', 'collect', 'out', 'relay', 'scheduled', 'new', 'accept', 'reject' ) );
            }else{
                return array_merge( $order_statuses, array( 'made', 'collect', 'out', 'relay', 'scheduled' ) );
            }
        }
    ...
    function bkf_status_bulk_actions($bulk_actions){
        ...
        $bulk_actions['mark_processing'] = __( 'Change status to received', 'bakkbone-florist-companion' );
		$bulk_actions['mark_scheduled'] = __( 'Change status to scheduled', 'bakkbone-florist-companion' );
		$bulk_actions['mark_made'] = __( 'Change status to prepared', 'bakkbone-florist-companion' );
		$bulk_actions['mark_collect'] = __( 'Change status to ready for collection', 'bakkbone-florist-companion' );
		$bulk_actions['mark_out'] = __( 'Change status to out for delivery', 'bakkbone-florist-companion' );
		$bulk_actions['mark_completed'] = __( 'Change status to delivered', 'bakkbone-florist-companion' );
		$bulk_actions['mark_relay'] = __( 'Change status to relayed', 'bakkbone-florist-companion' );
		return $bulk_actions;		
    }
