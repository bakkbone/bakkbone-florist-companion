	...
	function woocommerce_admin_field_bkf_weekday($value) {
		...
		<tr valign="top" class="petals_url"<?php echo $value['row_class'] ? ' class="' . esc_attr( $value['row_class'] ) . '"' : '' ?>">
			<th scope="row" class="titledesc">
				<label for="bkf-sd-<?php echo esc_attr($day); ?>"><?php echo esc_html( $value['title'] ); ?> <?php echo $tooltip_html; ?></label>
			</th>
			<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
		<div><label><input id="bkf-dd-<?php echo esc_attr($day); ?>" <?php echo $checked ?> type="checkbox" class="bkf-form-control" name="bkf_dd_setting[<?php echo esc_attr($day); ?>]" /><?php esc_html_e("Enabled","bakkbone-florist-companion") ?></label></div>
		<?php
		if($value['value']){
			$disabled = " required";
		} else {
		...
		?>
		<input class="form-control" id="bkf-sd-<?php echo esc_attr($day); ?>" type="time" step="60" name="bkf_sd_setting[<?php echo esc_attr($day); ?>]" placeholder="" value="<?php echo esc_attr($value['time']); ?>"<?php echo $disabled; ?> />
		<input class="small-text form-control" id="bkf-sd-<?php echo esc_attr($day); ?>lead" type="number" step="1" min="0" max="<?php echo get_option('bkf_ddi_setting')['ddi'] * 7 - 1; ?>" name="bkf_sd_setting[<?php echo esc_attr($day); ?>lead]" placeholder="" value="<?php echo esc_attr($value['lead']); ?>"<?php echo $disabled; ?> />
		</td></tr>
		<?php
	}
