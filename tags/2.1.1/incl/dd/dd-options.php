...
class BkfDdOptions{
	...
	function bkfAddDdiOptionsSanitize($input)
	{
		...
		if(isset($input["ddtst"])){
			$new_input["ddtst"] = sanitize_text_field($input["ddtst"]);
		}
	...
	function bkfDdtstCallback(){
		if(!isset($this->bkf_ddi_setting["ddtst"])){
			$this->bkf_ddi_setting["ddtst"] = false;
		}
		if($this->bkf_ddi_setting["ddtst"] == true){
			$checked = "checked";
		}else{
			$checked = "";
		}
		?>
		<label class="bkf-check-container"><?php _e("Time Slot Fees are taxable","bakkbone-florist-companion") ?><input id="bkf-ddtst" <?php echo $checked ?> type="checkbox" class="bkf-form-control" name="bkf_ddi_setting[ddtst]" /><span class="bkf-check-checkmark"></span></label>
		<?php
	}
