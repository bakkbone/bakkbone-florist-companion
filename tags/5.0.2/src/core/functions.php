...
function bkf_check_dd_availability($then, $method){
	...
	if (!$open) {
		$result = [false, 'closed', __('Closed', 'bakkbone-florist-companion')];
	} else {
		$result = [true];
		...
		if ($result[0] && $timenow >= $cutoff) {
			$result = [false, 'sdc', __('Order Cutoff Passed for this day', 'bakkbone-florist-companion')];
		}
		
		$msl = bkf_get_method_specific_leadtimes();
		if ($result[0]) {
			foreach ($msl as $lead) {
				if (strtolower($lead['day']) == $weekday and $lead['method'] == $method) {
					$mslcutoff = new DateTimeImmutable($date.'.'.$month.'.'.$year.' -'.$lead['leadtime'].' days '.$lead['cutoff'], $tz);
					if ($timenow >= $cutoff) {
						$result = [false, 'sdc', __('Order Cutoff Passed for some items in your order', 'bakkbone-florist-companion')];
					}
				}
			}
		}

		$methodday = isset(get_option('bkf_dm_setting')[$weekday]) ? get_option('bkf_dm_setting')[$weekday] : true;
		if ($result[0] && !$methodday) {
			$result = [false, 'closed', __('Closed', 'bakkbone-florist-companion')];
		}
		...
		if ($result[0]) {
			foreach ($full as $key => $value) {
				...
				if ($result && $fulltime == $then) {
					$result = [false, 'booked', __('Fully Booked', 'bakkbone-florist-companion')];
				}
			}
		}
		...
		if ($result[0]) {
			foreach ($catblocks as $block) {
				...
				if ($blocktime == $then && in_array($block['category'], $categories)) {
					$result = [false, 'unavailable', __('Some items in your order are unavailable on this date', 'bakkbone-florist-companion')];
				}
			}
		}
	}
