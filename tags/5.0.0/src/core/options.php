...
class BKF_Options{
	...
	private $bkf_audio_setting = [];
	private $bkf_advanced_setting = [];
	...
	function __construct(){
		...
		add_action("admin_menu", [$this, "bkfAddOptionsPageOption"], 2);
		add_action("admin_init", [$this, "bkfAddOptionsPageInit"]);
	}
	...
	function bkfAddOptionsPageOption(){
		add_menu_page(
			null,
			__("FloristPress","bakkbone-florist-companion"),
			"manage_woocommerce",
			"bkf_options",
			null,
			__BKF_SVG_FLOWERS__,
		...
		$admin_page = add_submenu_page(
			...
			"manage_woocommerce",
			"bkf_options",
			[$this, "bkfOptionsPageContent"],
			10
		);
		add_action( 'load-'.$admin_page, [$this, 'bkf_go_help_tab'] );
	}
	
	function bkf_go_help_tab(){
		$screen = get_current_screen();
		$id = 'bkf_go_help';
		$callback = [$this, 'bkf_go_help'];
	...
	function bkf_go_help(){
		...
		<h2><?php esc_html_e('View documentation for this page at: ','bakkbone-florist-companion'); ?></h2>
			<a href="https://docs.floristpress.org/florist-options/general-options/" target="_blank">https://docs.floristpress.org/florist-options/general-options/</a>
		<?php
	}
	...
	function bkfAddOptionsPageInit(){
		register_setting(
			"bkf_options_group",
			"bkf_options_setting",
			[$this, "bkfAddOptionsSanitize"]
		);
		register_setting(
			"bkf_options_group",
			"bkf_features_setting",
			[$this, "bkfAddFeaturesSanitize"]
		);
		register_setting(
			"bkf_options_group",
			"bkf_advanced_setting",
			[$this, "bkfAddAdvancedSanitize"]
		);
		...
		add_settings_field(
			"confirm_email",
			__("Confirm Email", "bakkbone-florist-companion"),
			[$this, "bkfCECallback"],
			"bkf-options",
			"bkf_features_section"
		);
		...
		add_settings_field(
			"autoprocess",
			__("Auto Process Orders", "bakkbone-florist-companion"),
			[$this, "bkfAPCallback"],
			"bkf-options",
			"bkf_features_section"
		);
		...
		add_settings_field(
			"settingsbar",
			__("Admin Bar", "bakkbone-florist-companion"),
			[$this, "bkfSBCallback"],
			"bkf-options",
			"bkf_features_section"
		);
	...
	function bkfAddFeaturesSanitize($input){
		...
		if(isset($input["autoprocess"])){
			$new_input["autoprocess"] = true;
		} else {
			$new_input["autoprocess"] = false;
		}
		
		if(isset($input["settingsbar"])){
			$new_input["settingsbar"] = true;
		} else {
			$new_input["settingsbar"] = false;
		}
		
		if(isset($input["confirm_email"])){
			$new_input["confirm_email"] = true;
		} else {
			$new_input["confirm_email"] = false;
		}
	...
	function bkfCECallback(){
	
		if(!isset($this->bkf_features_setting["confirm_email"])){
			$this->bkf_features_setting["confirm_email"] = false;
		}
		if($this->bkf_features_setting["confirm_email"] == true){
			$checked = "checked";
		} else {
			$checked = "";
		}
		?>
		<label class="bkf-check-container"><?php esc_html_e("Require email confirmation at checkout for logged-out users", "bakkbone-florist-companion") ?><input id="bkf-confirm-email" <?php echo $checked ?> type="checkbox" class="bkf-form-control"  name="bkf_features_setting[confirm_email]" /><span class="bkf-check-checkmark"></span></label>
		<?php
	}

	function bkfAPCallback(){
	
		if(!isset($this->bkf_features_setting["autoprocess"])){
			$this->bkf_features_setting["autoprocess"] = false;
		}
		if($this->bkf_features_setting["autoprocess"] == true){
			$checked = "checked";
		} else {
			$checked = "";
		}
		?>
		<label class="bkf-check-container"><?php esc_html_e("Automatically change all orders to Processed", "bakkbone-florist-companion") ?><input id="bkf-autoprocess" <?php echo $checked ?> type="checkbox" class="bkf-form-control"  name="bkf_features_setting[autoprocess]" /><span class="bkf-check-checkmark"></span></label>
		<?php
	}

	function bkfSBCallback(){
	
		if(!isset($this->bkf_features_setting["settingsbar"])){
			$this->bkf_features_setting["settingsbar"] = false;
		}
		if($this->bkf_features_setting["settingsbar"] == true){
			$checked = "checked";
		} else {
			$checked = "";
		}
		?>
		<label class="bkf-check-container"><?php esc_html_e("Add WooCommerce settings to the Admin Bar", "bakkbone-florist-companion") ?><input id="bkf-settingsbar" <?php echo $checked ?> type="checkbox" class="bkf-form-control"  name="bkf_features_setting[settingsbar]" /><span class="bkf-check-checkmark"></span></label>
		<?php
	}
