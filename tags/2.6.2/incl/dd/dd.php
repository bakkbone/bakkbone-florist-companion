...
class BkfDd{
    ...
    function __construct(){
		...
		add_action('wp_dashboard_setup', array($this, 'bkf_dashboard_widgets'));
    }
	
  
	function bkf_dashboard_widgets() {
		global $wp_meta_boxes;
		wp_add_dashboard_widget('bkf_dd', __("Today's Deliveries", "bakkbone-florist-companion"), array($this, 'bkf_dashboard_dd'));
	}
 
	function bkf_dashboard_dd() {
		echo '<ul>';
		...
		foreach($orders as $order){
			...
			echo '<li style="border:1px solid black;padding:5px;margin:0 0 5px"><a href="'.$url.'"><strong>#'.$on." - ". $suburb.'</strong></a><br>'.$recipient.' - <a href="tel:'.$phone.'">'.$phone.'</a></li>';			
		}
		echo '</ul>';
	}
    ...
    function bkf_dd_checkout_validation( $fields, $errors ){
		...
		if( $has_physical ) {
			...
			if($_POST['delivery_date'] !== ''){
				...
				$tscol = array_column($ts, 'day');
				$validts = array_search($day, $tscol);
			} else {
				$validts = array();
			}
			$currentshipping = WC()->session->get( ‘chosen_shipping_methods’ );
			foreach($validts as $thists){
				if(!in_array($thists['method'], $currentshipping)){
					unset($thists);
				}
			}
