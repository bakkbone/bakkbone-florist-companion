...
class BkfDdFilter{
	...
	function bkf_custom_filters($views){
		...
		$allstatus = array("wc-processing","wc-made","wc-collect","wc-out","wc-relay","wc-scheduled","wc-new","wc-accept","wc-payment-await","wc-on-hold");
		...
		if($post_type == 'shop_order'){
			if($show_all == 'true'){
				$views['all'] = sprintf($text_all_inactive, $url_to_redirect_all, $this->all_count());
				$views['full'] = sprintf($text_full_active, $url_to_redirect_full, $this->full_count());
			} elseif ($post_status == '' || $post_status == null){
				$views['all'] = sprintf($text_all_active, $url_to_redirect_all, $this->all_count());
				$views['full'] = sprintf($text_full_inactive, $url_to_redirect_full, $this->full_count());
			} else {
				$views['all'] = sprintf($text_all_inactive, $url_to_redirect_all, $this->all_count());
				$views['full'] = sprintf($text_full_inactive, $url_to_redirect_full, $this->full_count());
			}
