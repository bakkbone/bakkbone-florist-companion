...
class BkfDdOptions{
    ...
    function bkf_admin_menu(){
		add_menu_page(
			null, //$page_title
			__("Delivery Dates","bakkbone-florist-companion"), //$menu_title
        ...
        __("General Options","bakkbone-florist-companion"),//menu title
        "manage_options",//capability
        "bkf_dd",//menu slug
        ...
        "manage_options",//capability
        "bkf_dd_wd",//menu slug
        array($this, "bkf_wd_settings_page"),//callback
        2
        );
        $sdpage = add_submenu_page(
        "bkf_dd",//parent slug
        __("Same Day Delivery Cutoffs","bakkbone-florist-companion"),//page title
        __("Same Day","bakkbone-florist-companion"),//menu title
        "manage_options",//capability
        "bkf_dd_sd",//menu slug
        array($this, "bkf_sd_settings_page"),//callback
        3
        );
        $dmpage = add_submenu_page(
        "bkf_dd",//parent slug
        __("Delivery Method Restrictions","bakkbone-florist-companion"),//page title
        __("Methods","bakkbone-florist-companion"),//menu title
        "manage_options",//capability
        "bkf_dd_dm",//menu slug
        array($this, "bkf_dm_settings_page"),//callback
        4
        );
	...
	function bkf_dd_help(){
		...
		<h2><?php echo BKF_HELP_SUBTITLE; ?></h2>
			<a href="https://docs.bkbn.au/v/bkf/delivery-dates/general-options" target="_blank">https://docs.bkbn.au/v/bkf/florist-options/pdfs</a>
		<?php
	}
	...
	function bkfAddDdPageInit()
	{
		...
		register_setting(
			"bkf_sd_options_group",// group
			"bkf_sd_setting", //setting name
			array($this,"bkfAddSdOptionsSanitize") //sanitize_callback
		);
		register_setting(
			"bkf_dm_options_group",// group
			"bkf_dm_setting", //setting name
			array($this,"bkfAddDmOptionsSanitize") //sanitize_callback
		);
		...
		<select name="delivery_timeslot" style="width:100%;" class="delivery_timeslot form-control" id="delivery_timeslot">
			...
			foreach($ts as $tslot){
				if($tslot['day'] == strtolower(date("l", strtotime($delivery_date))) && $tslot['method'] == $method ){
					$stringts = date("g:i a", strtotime($tslot['start'])).' - '.date("g:i a", strtotime($tslot['end']));
