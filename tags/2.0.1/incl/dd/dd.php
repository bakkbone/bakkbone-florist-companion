...
class BkfDd{
    ...
    function bkf_dd_order ( $order_id ) {
    	...
    	if ( isset( $_POST ['delivery_timeslot'] ) &&  '' != $_POST ['delivery_timeslot'] ) {
    		update_post_meta( $order_id, '_delivery_timeslot',  sanitize_text_field( $_POST ['delivery_timeslot'] ) );
    	}
    }
    ...
    function bkf_dd_to_email ( $fields, $sent_to_admin, $order ) {
		...
		$thistsid = array_search($rawtimeslot, $tscol);
		$thists = $ts[$thistsid];
		$delivery_timeslot = date("g:i a", strtotime($thists['start'])).' - '.date("g:i a", strtotime($thists['end']));
    ...
    function bkf_dd_thankyou ( $order ) {
		...
		$thists = $ts[$thistsid];
        if ( '' !== $delivery_timeslot ) {
        	echo '<p><strong>' . $tstitle . ':</strong><br>' . date("g:i a", strtotime($thists['start'])).' - '.date("g:i a", strtotime($thists['end'])) . '</p>';
    	}
    }
    ...
    function bkf_dd_checkout_validation( $fields, $errors ){
		...
		$dayname = strtolower(date("l",strtotime($_POST['delivery_date'])));
		$validts = array();
		foreach($ts as $tslot){
			if($tslot['day'] == $dayname && $tslot['method'] == implode($_POST['shipping_method'])){
				$validts[] = $tslot['id'];
			}
		}
		$ddtitle = get_option('bkf_ddi_setting')['ddt'];
	...
	function bkf_dd_col( $column, $post_id ) {
	    ...
	    switch ( $column )
	    {
		   ...
		   if($timeslotvalue !== null){
			   foreach($ts as $tslot){
				   if( in_array($tslot['id'],$timeslotvalue)){
					   echo '<br>'.date("g:i a", strtotime($tslot['start'])).' - '.date("g:i a", strtotime($tslot['end']));
				   } else {}
			   }
		   }
	    }
