 ...
 * @package BKF_Petals_CPT
 * @license GNU General Public License (GPL) 3.0
**/

defined("__BKF_EXEC__") or die("Ah, sweet silence.");

class BKF_Petals_CPT{

	function __construct(){
		$bkfoptions = get_option("bkf_features_setting");
		if($bkfoptions["petals_on"]) {
			add_action("init", [$this, 'bkfRegisterPetalsOrderPost']);
			add_action("load-post.php", [$this, 'popost_help_tab']);
			add_action("load-post-new.php", [$this, 'popost_help_tab']);
			add_action("load-edit.php", [$this, 'popost_help_tab']);
			add_action('init', [$this, 'bkfmakePrintContentsSaySaved']);
			add_action('acf/init', [$this, 'bkf_acf_add_local_field_groups']);
			add_action('acf/save_post', [$this, 'bkf_save_po'], 10, 3);
			add_filter('single_template', [$this, 'bkf_po_template']);
			add_filter('post_row_actions', [$this, 'bkf_qe'], 10, 2);
			add_filter('pre_get_document_title', [$this, 'bkf_po_title']);
			add_filter('get_edit_post_link', [$this, 'bkf_po_edit_link'], 10, 3);
			add_filter('manage_bkf_petals_order_posts_columns', [$this, 'bkf_po_table']);
			add_action('manage_bkf_petals_order_posts_custom_column', [$this, 'bkf_po_table_content'], 10, 2);
			add_filter('manage_edit-bkf_petals_order_sortable_columns', [$this, 'bkf_po_col_sort'], 10, 1 );
			add_action('pre_get_posts', [$this, 'bkf_po_filter'] );
			add_filter('bulk_actions-edit-bkf_petals_order', 'removeBulkActionsInput' );
			if(is_admin()){
				add_filter('comments_clauses', [$this, 'bkf_exclude_order_comments'], 10, 1 );
				add_filter('comment_feed_where', [$this, 'exclude_order_comments_from_feed_where'] );
			}
		}
	}
	...
	function bkf_po_edit_link($link, $postid, $context){
	...
	function popost_help_tab(){
		...
		if($screen->id == 'edit-bkf_petals_order' || $screen->id == 'bkf_petals_order'){
			$id = 'petalsorder_help';
			$callback = [$this, 'petalsorder_help'];
	...
	function petalsorder_help(){
		...
		<h2><?php esc_html_e('View documentation for this page at: ','bakkbone-florist-companion'); ?></h2>
			<a href="https://docs.floristpress.org/petals-network/petals-network-integration/" target="_blank">https://docs.floristpress.org/petals-network/petals-network-integration/</a>
		<?php
	}
