...
class BKF_Admin_Notices{
	...
	function admin_bar_item($admin_bar){
		...
		$admin_bar->add_menu( array(
			'id'	=> 'floristpress',
			'parent'=> null,
			'title'	=> '<span class="bkf_flowers_icon">'.__BKF_SVG_FLOWERS_RAW__.'</span><span class="ab-label">'.__( 'FloristPress', 'bakkbone-florist-companion' ).'</span>',
			'meta'	=> [
		...
		$admin_bar->add_node([
			...
			'title'		=> '<input type="text" id="bkf_adminbar_ordersearch" placeholder="'.esc_html__('Search orders...', 'bakkbone-florist-companion').'" /><div id="bkf_ordersearch_results" style="display:none;"></div>',
		]);
	}
	...
	function notices(){
		...
		foreach($all_plugins as $plugin){
			...
			if(isset($plugin['BKF tested up to']) && version_compare(__BKF_VERSION__, $tested, 'gt')){
				$untested[] = $plugin;
			}
			if(isset($plugin['BKF requires at least']) && version_compare($requires, __BKF_VERSION__, 'gt')){
				$incompatible[] = $plugin;
			}
		}

		foreach($untested as $offender){
			/* translators: %1$s: conflicting plugin name. %2$s: this plugin name. */
			echo '<div class="notice-error notice"><p>'. sprintf(__('<strong>%1$s</strong> has not been tested with your current version of %2$s – please ask the developer to update their plugin.','bakkbone-florist-companion'), $offender['Title'], $bkftitle).'</p></div>';
		}
		foreach($incompatible as $offender){
			/* translators: %1$s: conflicting plugin name. %2$s: Required version of this plugin. %3$s: this plugin name. */
			echo '<div class="notice-error notice"><p>'. sprintf(__('<strong>%1$s</strong> requires at least version <strong>%2$s</strong> of %3$s – please update %3$s.','bakkbone-florist-companion'), $offender['Title'], $offender['BKF requires at least'], $bkftitle).'</p></div>';
		}
		if(bkf_is_woocommerce_active()){
			$checkout_page = wc_get_page_id('checkout');
			$block_enabled = has_block('woocommerce/checkout', $checkout_page);
		...
		if($features['excerpt_pa'] && bkf_is_breakdance_active()){
			...
			echo '<div class="notice-error notice"><p>'. sprintf(__('<strong>%1s</strong> already includes the capability to show Short Descriptions in product archives – please disable the %2s Short Description feature %3shere%4s.','bakkbone-florist-companion'), $bdtitle, $plugin_data["Name"], '<a href="'.bkf_wc_admin_link().'">', '</a>').'</p>
			</div>';
		}
	}
	...
	function dashblocks(){
		...
		echo '<h3 style="text-align:center;"><strong>'.wp_kses_post(sprintf(__('View and manage your blocked dates in more detail %1shere%2s'), '<a href="'.bkf_wc_admin_link('bkf_dd', 'ddb').'">','</a>')).'</strong></h3>';
		?>
