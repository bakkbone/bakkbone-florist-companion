...
class BKF_Core {
	...
	function __construct() {
		...
		add_filter( 'woocommerce_get_country_locale', [$this, 'woocommerce_get_country_locale'], PHP_INT_MAX, 1);
		...
		add_filter( 'woocommerce_billing_fields' , [$this, 'bkf_override_billing_fields'], PHP_INT_MAX );
		add_filter( 'woocommerce_shipping_fields' , [$this, 'bkf_override_shipping_fields'], PHP_INT_MAX);
	...
	function add_card_message_field( $fields ) {
		...
		if( bkf_cart_has_physical() && bkf_not_nonfloral() ) {
	...
	function woocommerce_get_country_locale($locales) {
		foreach ($locales as &$locale) {
			if (isset($locale['state']) && isset($locale['state']['label'])) {
				$locale['state']['label'] = get_option('bkf_localisation_setting')['global_label_state'];
			}
			if (isset($locale['city']) && isset($locale['city']['label'])) {
				$locale['city']['label'] = get_option('bkf_localisation_setting')['global_label_suburb'];
			}
			if (isset($locale['postcode']) && isset($locale['postcode']['label'])) {
				$locale['postcode']['label'] = get_option('bkf_localisation_setting')['global_label_postcode'];
			}
		}
		return $locales;
	}
