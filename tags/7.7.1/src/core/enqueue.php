...
class BKF_Scripts {
	...
	function backend(){
		...
		if(isset($_GET["tab"]) && isset($_GET["section"])) {
		    if($_GET["tab"] == "bkf_dd" && ($_GET["section"] == "ddb" || $_GET["section"] == "cb")) {
        		wp_enqueue_script("bkf_form", __BKF_URL__ . "assets/js/form{$min}.js", '', __BKF_VERSION__, 'all');
        		wp_localize_script("bkf_form","formlang", [
        		    'compareDates'  => esc_html__('Start date must be earlier than end date.', 'bakkbone-florist-companion'),
        		    'emptyField'  => esc_html__('Please complete all fields to submit this form.', 'bakkbone-florist-companion')
        		]);
		    }
		}
	}
