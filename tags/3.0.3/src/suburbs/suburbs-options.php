	...
	function bkf_suburbs_page(){
		...
		<div class="wrap">
			...
			<div style="columns: 500px 3;width:100%;">
				<?php foreach($sm as $smethod){if(!$smethod['pickup']){?><div class="inside bkf-inside" style="break-inside:avoid;margin-top:0;">
					...
					<p style="margin-top:0;text-align:center;color:black;"><strong><?php esc_html_e('Zone', 'bakkbone-florist-companion'); ?>: </strong><?php echo $smethod['zone']; ?><br><strong><?php esc_html_e('Area', 'bakkbone-florist-companion'); ?>: </strong><?php echo $smethod['zonelocation']; ?></p>
						...
						if(empty($smid[$smethod['rateid']])){
							echo '<div class="bkf-info"><p>'.esc_html__('This delivery method has no suburbs attached, and will be available for any suburb in its delivery Zone.', 'bakkbone-florist-companion').'</p></div>';
						} else {
							echo '<ul class="bkf-list" style="columns: 200px auto;">';
						foreach($sub as $suburbitem){
							if($suburbitem['method'] == $smethod['rateid']){
									echo '<li>'.$suburbitem['suburb'].' <em><a href="'.admin_url('admin-ajax.php?action=bkf_suburb_del&nonce='.$nonce.'&id='.$suburbitem['id']).'">'.esc_html__('Delete','bakkbone-florist-companion').'</a></em></li>';
							}
						}
						echo '</ul>';
					}
