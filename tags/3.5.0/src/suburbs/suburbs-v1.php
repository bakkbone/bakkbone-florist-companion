...
class BKF_Suburbs_Core_v1{
	...
	function __construct(){
		...
		register_activation_hook( __FILE__, [$this, 'bkf_suburbs_db_init']);
		$this->bkf_suburbs_setting = get_option("bkf_suburbs_setting");
		add_action("plugins_loaded", [$this,"bkf_update_suburbs_db_check"]);
		add_action("admin_menu", [$this,"bkf_suburbs_admin_menu"],40);	
		add_filter('woocommerce_package_rates', [$this, 'bkf_del_suburb_methods'], PHP_INT_MAX, 2);
	}
	...
	function bkf_suburbs_admin_menu(){
		...
		"manage_woocommerce",
		"bkf_suburbs",
		[$this, 'bkf_suburbs_page'],
		40
		);
		add_action( 'load-'.$admin_page, [$this, 'bkf_ds_help_tab'] );
	}
	
	function bkf_ds_help_tab(){
		$screen = get_current_screen();
		$id = 'bkf_ds_help';
		$callback = [$this, 'bkf_ds_help'];
	...
	function bkf_ds_help(){
		...
		<h2><?php esc_html_e('View documentation for this page at: ','bakkbone-florist-companion'); ?></h2>
			<a href="https://docs.floristpress.org/florist-options/delivery-suburbs-classic/" target="_blank">https://docs.floristpress.org/florist-options/delivery-suburbs-classic/</a>
		<?php
	}
