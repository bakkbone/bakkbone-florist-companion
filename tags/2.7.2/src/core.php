...
class BkfCore{
	...
	function __construct() {
		...
		add_action('shutdown', array($this, 'bkf_sig_shutdown'), 999);
    ...
    function bkf_settings_link( $links ) {
    	...
    	$docs_link = "<a href='https://plugins.bkbn.au/docs/bkf/'>" . __('Documentation','bakkbone-florist-companion') . '</a>';
    ...
    function bkf_ot_thankyou ( $order ) {
        ...
        if ( '' !== $cm ) {
        	echo '<p><strong>' . __('Card Message', 'bakkbone-florist-companion') . ':</strong><br>' . $cm . '</p>';
    	}
    ...
    function bkf_cd_thankyou ( $order ) {
        ...
        if ( '' !== $sn ) {
        	echo '<p><strong>' . __('Delivery Notes', 'bakkbone-florist-companion') . ':</strong><br>' . $sn . '</p>';
    	}
    ...
    public function bkf_cm_metabox_callback( $post ){
        ...
        ?><textarea style="font-family:monospace;width:100%;" name="card_message" rows="6" class="card_message input-text form-control" id="card_message" maxlength="<?php echo get_option('bkf_options_setting')['card_length'] ?>" placeholder="<?php _e('Card Message', 'bakkbone-florist-companion'); ?>"><?php echo esc_html( $cardmessage ) ?></textarea>
    	<?php  
    }
		...
		?>
			<div class="address">
				<p<?php if( empty( $shippingnotes ) ) { echo ' class="none_set"'; } ?>>
		 			<strong><?php _e('Delivery Notes', 'bakkbone-florist-companion'); ?>:</strong>
