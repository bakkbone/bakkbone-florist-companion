      ...
      document.addEventListener('DOMContentLoaded', function() {
		...
		foreach($orders as $order){
			...
			if(get_post_meta($id,'_delivery_timestamp',true) !== ''){
				...
				$customer = get_post_meta($order->ID, '_billing_first_name',true).' '.get_post_meta($order->ID, '_billing_last_name',true);
				...
				if($orderObj->needs_shipping_address()){
					if(get_post_meta($id,'_delivery_timeslot_id',true) !== ''){
						...
						$tsid = get_post_meta($id,'_delivery_timeslot_id',true);
						$ts = $allts[$tsid];
						$start = $ts['start'];
						$end = $ts['end'];
					...
					if(get_post_meta($id,'_delivery_timeslot_id',true) !== ''){
						...
						$start = $ts['start'];
						$end = $ts['end'];
						echo '{ title: "#'.$order->ID.' - '.__('Pickup', 'bakkbone-florist-companion').', '.$customer.'", url: "'.html_entity_decode($url).'", start: "'.$date.' '.$start.':00", end: "'.$date.' '.$end.':00", classNames: ["unshipped"] }, ';
					} else {
						echo '{ title: "#'.$order->ID.' - '.__('Pickup', 'bakkbone-florist-companion').', '.$customer.'", url: "'.html_entity_decode($url).'", start: "'.$date.'", classNames: ["unshipped"] }, ';
					}
				}
