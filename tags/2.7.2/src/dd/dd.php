...
class BkfDd{
	...
	function bkf_delivery_details(){
	    echo '<h3>'.__('Delivery details', 'bakkbone-florist-companion').'</h3>';
	}
    ...
    function bkf_dd_field_init () {
	    	...
	    	<script id="bkf_ddfield">
	    	    jQuery(document).ready(function( $ ) {
	 			...
	 			<?php if(get_option('bkf_dd_setting')['monday'] == false){ ?>
	 				if (date >= new Date("<?php echo bkf_get_monday(); ?>") && w == 1) { return [false, "closed", "<?php _e('Closed', 'bakkbone-florist-companion'); ?>"]; }<?php } else {
	 				if(isset(get_option('bkf_dm_setting')['monday'])){
	 					...
	 					if(w == 1 && monday_items.includes(currentShippingMethod)){
	 						return [false, "unavailable", '<?php _e('Unavailable', 'bakkbone-florist-companion'); ?>'];
	 					}
	 					<?php
	 				}
	 			}; ?>
	 			<?php if(get_option('bkf_dd_setting')['tuesday'] == false){ ?>
	 				if (date >= new Date("<?php echo bkf_get_monday(); ?>") && w == 2) { return [false, "closed", "<?php _e('Closed', 'bakkbone-florist-companion'); ?>"]; }<?php } else {
	 			...
	 			<?php if(get_option('bkf_dd_setting')['wednesday'] == false){ ?>
	 				if (date >= new Date("<?php echo bkf_get_monday(); ?>") && w == 3) { return [false, "closed", "<?php _e('Closed', 'bakkbone-florist-companion'); ?>"]; }<?php } else {
	 			...
	 			<?php if(get_option('bkf_dd_setting')['thursday'] == false){ ?>
	 				if (date >= new Date("<?php echo bkf_get_monday(); ?>") && w == 4) { return [false, "closed", "<?php _e('Closed', 'bakkbone-florist-companion'); ?>"]; }<?php } else {
	 			...
	 			<?php if(get_option('bkf_dd_setting')['friday'] == false){ ?>
	 				if (date >= new Date("<?php echo bkf_get_monday(); ?>") && w == 5) { return [false, "closed", "<?php _e('Closed', 'bakkbone-florist-companion'); ?>"]; }<?php } else {
	 			...
	 			<?php if(get_option('bkf_dd_setting')['saturday'] == false){ ?>
	 				if (date >= new Date("<?php echo bkf_get_monday(); ?>") && w == 6) { return [false, "closed", "<?php _e('Closed', 'bakkbone-florist-companion'); ?>"]; }<?php } else {
	 			...
	 			<?php if(get_option('bkf_dd_setting')['sunday'] == false){ ?>
	 				if (date >= new Date("<?php echo bkf_get_monday(); ?>") && w == 0) { return [false, "closed", "<?php _e('Closed', 'bakkbone-florist-companion'); ?>"]; }<?php } else {
			 ...
			 for (i = 0; i < closedDatesList.length; i++) {
			   ...
			   {
			   	 return [false,"closed","<?php _e('Closed', 'bakkbone-florist-companion'); ?>"];
			   }
			 }
		     for (i = 0; i < fullDatesList.length; i++) {
		       if ((m == fullDatesList[i][0] - 1) && (d == fullDatesList[i][1]) && (y == fullDatesList[i][2]))
		       {
		         return [false,"booked","<?php _e('Fully Booked', 'bakkbone-florist-companion'); ?>"];
		       }
		     }
			 ...
			 <?php if($sdcpassed == true){
				 ...
				 if(d == "<?php echo wp_date("j"); ?>" && y == "<?php echo wp_date("Y"); ?>" && m == <?php echo wp_date("n"); ?> - 1){
					 return [false,"sdc","<?php _e('Same Day Delivery Cutoff Passed', 'bakkbone-florist-companion'); ?>"];
					 }<?php
			 }?>
        ...
        <?php _e( "Timeslot: ", "bakkbone-florist-companion"); ?>
    	<abbr class="required" title="required">*</abbr></label>
    ...
    function bkf_dd_order ( $order_id ) {
    	...
    	if ( isset( $_POST ['delivery_timeslot'] ) &&  '' != $_POST ['delivery_timeslot'] ) {
			...
			$text = bkf_get_timeslot_string($thistsid);
		...
		if( bkf_cart_has_physical() ) {
			...
			if($_POST['delivery_date'] !== ''){
				...
				foreach($timeslots as $timeslot){
                    if($timeslot->day == $day && in_array($timeslot->method, $currentshipping)){
                                        $validts[] = array(
                            'id'		=>	$timeslot->id,
