		...
		if(count($orders) == 0){
			...
			foreach($orders as $order){
				...
				if($shipping){
					...
					echo '<li><h3><a href="'.$url.'"><strong>#'.$on.' - '.__('Pickup', 'bakkbone-florist-companion').', '.$customer.'</h3><p><strong><a href="'.$wsurl.'">'.get_option('bkf_pdf_setting')['ws_title'].'</a></strong></p></li>';			
				} else {
		...
		if(count($recentorders) == 0){
			...
			foreach($recentorders as $wcorder){
				...
				if($shipping){
					...
					$text = '<li><h3><a href="'.$url.'" target="_blank"><strong>#'.$on.' - '.__('Pickup', 'bakkbone-florist-companion').', '.$value.'</strong></a></h3><p>'.$dd.'</p><p><strong><a href="'.$wsurl.'">'.get_option('bkf_pdf_setting')['ws_title'].'</a></strong></p></li>';
				} else {
