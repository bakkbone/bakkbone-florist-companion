...
<notes>'.$msgbody.'</notes>
        ...
        $response = wp_remote_post($url, array(
	        ...
	        if($xml->type == '300'){
                $note = '<strong>'.__('Message successfully sent to Petals:', 'bakkbone-florist-companion').' </strong><br>' . $msgbody . '<br><br><strong>'.__('Response from Petals:', 'bakkbone-florist-companion').'</strong> <br><strong>' . $implosion;
                ...
                $note = '<strong>'.__('Message FAILED TO SEND to Petals. Your message:', 'bakkbone-florist-companion').' </strong><br>' . $msgbody . '<br><br>'.__('Response from Petals:', 'bakkbone-florist-companion').' <br><strong>' . $implosion;
				$comment_author_email  = 'bkf@';
				$comment_author_email .= isset( $_SERVER['HTTP_HOST'] ) ? str_replace( 'www.', '', sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) ) ) : 'noreply.com';
				$comment_author_email  = sanitize_email( $comment_author_email );
				$commentargs = array(
...
<notes>'.$msgbody.'</notes>
    ...
    function bkf_petals_outbound_ajax(){
        ...
        if($xml->recordtype == 3){
            ...
            $note = '<strong>'.__('Message received from Petals:', 'bakkbone-florist-companion') . ' </strong><br>' . (string)$xml->notes;
