<?php
/**
 * Class WC_Email_Petals_Message file
 *
...
if ( ! class_exists( 'WC_Email_Petals_Message' ) ) :

	/**
	 * Petals Message Email.
	 *
	 * An email sent to the admin when a new Petals Message is received on an inbound order (order from merchant to Petals).
	 *
	 * @class       WC_Email_Petals_Message
	 * @version     2.0.0
	 * @package     WooCommerce\Classes\Emails
	 * @extends     WC_Email
	 */
	class WC_Email_Petals_Message extends WC_Email {
		...
		public function __construct() {
			...
			$this->title          = __( 'Petals - Message (Inbound Order)', 'bakkbone-florist-companion' );
			$this->description    = __( 'Message (Inbound Order) emails are sent to chosen recipient(s) when a message from Petals is received regarding an order you sent.', 'bakkbone-florist-companion' );
			$this->template_html  = 'emails/petals-message.php';
			$this->template_plain = 'emails/plain/petals-message.php';
		...
		public function get_default_subject() {
			return __( '[{site_title}]: Petals Order #{order_number} - Petals Message', 'bakkbone-florist-companion' );
		}
		...
		public function get_default_heading() {
			return __( 'Petals Message for order: #{order_number}', 'bakkbone-florist-companion' );
		}
		...
		public function trigger( $order_id, $comment_id ) {
			...
			$this->placeholders['{order_number}'] = get_post_meta($order_id, '_petals_on', true);
