						...
						if($wds[$day] == false){
							...
							if(!empty($dms[$day]) && in_array($smethod['rateid'],$dms[$day])){
								...
								if(!empty($co[$smethod['rateid'].'-'.$day])){
									...
									echo '<p><strong>'.ucwords($thisco['day']).':</strong> '.date("g:i a", strtotime($thisco['cutoff'])).' <em><a href="'.admin_url('admin-ajax.php?action=bkf_sd_del&nonce='.$nonce.'&id='.$thisco['id']).'">'.esc_html__('Reset','bakkbone-florist-companion').'</a></em></p>';
								} else {
