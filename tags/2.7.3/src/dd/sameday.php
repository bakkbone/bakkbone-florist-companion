					...
					<?php foreach($wd as $day){
						if($wds[$day] == false){
							...
							if(!empty($dms[$day]) && in_array($smethod['rateid'],$dms[$day])){
									echo '<p>'.sprintf(__('%s disabled for this method','bakkbone-florist-companion'), ucwords($day)).'</p>';
							} else {
								if(!empty($co[$smethod['rateid'].'-'.$day])){
									$thisco = $co[$smethod['rateid'].'-'.$day];
									echo '<p><strong>'.ucwords($thisco['day']).':</strong> '.date("g:i a", strtotime($thisco['cutoff'])).' <em><a href="'.admin_url('admin-ajax.php?action=bkf_sd_del&nonce='.$delnonce.'&id='.$thisco['id']).'">'.esc_html__('Reset','bakkbone-florist-companion').'</a></em></p>';
								} else {
