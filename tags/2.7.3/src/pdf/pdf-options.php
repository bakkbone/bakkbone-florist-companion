...
class BkfPdfOptions{
    ...
    function __construct(){
		...
		add_action('admin_footer', array($this,'header') );
	...
	function add_pdf_downloads( $order ) {
		...
		$wsnonce = wp_create_nonce("bkf_worksheet_pdf");
		$wsurl = admin_url( 'admin-ajax.php?action=bkfdw&order_id=' . $order_id . '&nonce=' . $wsnonce );
		if($petalson == null){
			echo '<p class="form-field form-field-wide wc-customer-user"><a href="'.$invurl.'">'.esc_html__('Download', 'bakkbone-florist-companion').' '.$invtitle.'</a></p>';
		}
		echo '<p class="form-field form-field-wide wc-customer-user"><a href="'.$wsurl.'">'.esc_html__('Download', 'bakkbone-florist-companion').' '.$wstitle.'</a></p>';
	}
