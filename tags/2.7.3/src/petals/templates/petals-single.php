	...
	if(current_user_can( 'read_petals_order')){
		...
		<div id="bkf_pm" name="bkf_pm">
			<p><div class="bkf-select" style="width:50%;"><select required form="bkf_pm" id="petals_msg_type" name="msgtype">
				...
				<option value="D"><?php echo esc_html__("Mark order as delivered", "bakkbone-florist-companion"); ?></option>
			</select></div></p>
			<p><input type="text" required class="bkf-form-control big" form="bkf_pm" name="msg" id="petals_msg_body" placeholder="<?php echo esc_html__("Message to Petals","bakkbone-florist-companion"); ?>" /></p>
			<p><button form="bkf_pm" class="button wp-element-button" onclick="ajaxPetalsMessage()"><?php echo esc_html__("Send Message","bakkbone-florist-companion"); ?></button></p>
		</div>
		<script>
			function ajaxPetalsMessage( $ ) {
				...
				function ajaxPetalsMessageProceed( $ ) {
					...
					alert('<?php echo esc_html__("Message queued for sending - this page will now attempt to refresh to display the outcome in the messages. An email will also be sent to the site admin.", "bakkbone-florist-companion"); ?>');
					location.reload();					
				}
		</script><?php
		if($commentscount !== 0){
