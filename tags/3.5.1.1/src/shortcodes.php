...
class BKF_Shortcodes{
	...
	function bkf_suburb_search($atts, $content, $tag){
		...
		$content= '<div class="bkf-suburb-search">
			<form>
		 		<input type="text" class="bkf-suburb-search-input" placeholder="'.$placeholder.'" onkeyup="bkfSuburbSearchShowResult(this.value)">
	...
	function mce_external_plugins($plugin_array){
		$plugin_array["bkf_page_title"] = BKF_URL . "/assets/tinymce-plugins/bkf_page_title.js";
		$plugin_array["bkf_site_title"] = BKF_URL . "/assets/tinymce-plugins/bkf_site_title.js";
		return $plugin_array;
	}

	function admin_enqueue_scripts(){
		wp_register_style("bkf_tinymce", BKF_URL . "/assets/tinymce-plugins/bkf_tinymce.css",[],"1.0.1" );
		wp_enqueue_style("bkf_tinymce");
	}
	
	function wp_enqueue_scripts(){
	    wp_enqueue_script( 'bkf_suburb_search', BKF_URL . '/assets/js/suburb_search.js', [], '1.0.1' );

	    // Add inline script to pass PHP variable to JS file
	    $url = admin_url('admin-ajax.php?action=bkf_search_suburbs_frontend&query=');
	    $inline_script = 'var ajaxUrl = "' . $url . '";';
