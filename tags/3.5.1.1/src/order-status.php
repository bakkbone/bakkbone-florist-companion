...
class BKF_Order_Status{
	...
	public function __construct() {
		add_filter( 'woocommerce_valid_order_statuses_for_payment', [$this, 'woocommerce_valid_order_statuses_for_payment'], 10, 2  );
		...
		add_filter( 'woocommerce_admin_order_actions', [$this, 'bkf_order_actions'], 10, 2 );
		add_filter( 'wc_order_statuses', [$this, 'bkf_rename_order_status_msg'], 20 );
		add_filter( "views_edit-shop_order", [$this, 'bkf_order_status_top_changed'], PHP_INT_MAX);
		add_filter( "views_woocommerce_page_wc-orders", [$this, 'bkf_order_status_top_changed'], PHP_INT_MAX);
		add_filter( 'woocommerce_menu_order_count', [$this, 'bkf_woocommerce_menu_order_count'], 10, 1 );
	}
	
	function woocommerce_valid_order_statuses_for_payment($statuses, $order){
		$statuses[] = 'invoiced';
		$statuses[] = 'phone-draft';
		return $statuses;
	}
	...
	public function bkf_register_order_status(){
		register_post_status('wc-processed', array(
			'label' => __('Processed', 'bakkbone-florist-companion'),
			'public' => true,
			'show_in_admin_status_list' => true,
			'show_in_admin_all_list' => true,
			'exclude_from_search' => false,
			'label_count' => _n_noop('Processed <span class="count">(%s)</span>', 'Processed <span class="count">(%s)</span>', 'bakkbone-florist-companion')
		));
		register_post_status('wc-collected', array(
			'label' => __('Collected', 'bakkbone-florist-companion'),
			'public' => true,
			'show_in_admin_status_list' => true,
			'show_in_admin_all_list' => true,
			'exclude_from_search' => false,
			'label_count' => _n_noop('Collected <span class="count">(%s)</span>', 'Collected <span class="count">(%s)</span>', 'bakkbone-florist-companion')
		));
	...
	public function bkf_add_status( $order_statuses ){
		...
		foreach ( $order_statuses as $key => $status ) {
			...
			if ( 'wc-processing' === $key ) {
				...
				$new_order_statuses['wc-processed'] = _x('Processed', 'Order status', 'bakkbone-florist-companion');
				$new_order_statuses['wc-collected'] = _x('Collected', 'Order status', 'bakkbone-florist-companion');
			}
		}
	...
	function bkf_status_bulk_actions($actions){
		...
		$bulk_actions['mark_collected'] = __( 'Mark as collected', 'bakkbone-florist-companion' );
		$bulk_actions['mark_processed'] = __( 'Mark as processed', 'bakkbone-florist-companion' );
	...
	function bkf_order_actions( $actions, $order ) {
		...
		if ( $order->has_status( array( 'processing', 'scheduled', 'made', 'collect' ) ) ) {
			$default_actions['collected'] = array(
				'url'		=> wp_nonce_url( admin_url( 'admin-ajax.php?action=woocommerce_mark_order_status&status=collected&order_id=' . $order_id ), 'woocommerce-mark-order-status' ),
				'name'   	=> __( 'Collected', 'bakkbone-florist-companion' ),
				'action' 	=> 'view collected'
				);
		}
		if ( $order->has_status( array( 'processing' ) ) ) {
			$default_actions['processed'] = array(
				...
				'name'   	=> __( 'Processed', 'bakkbone-florist-companion' ),
	...
	echo '<style id="bkf_order_status">
		...
		mark.processed::after {
			content: "\\e015";
			color: #000055;
		}
		.view.processed::after {
			font-family: WooCommerce !important;
			color: #000055 !important;
			content: "\\e015" !important;
		}
		mark.order-status.status-collected {
			color: #ffffff;
			background-color: #000099;
		}
		mark.collected::after {
			content: "\\e015";
			color: #000099;
		}
		.view.collected::after {
			font-family: WooCommerce !important;
		...
		mark.scheduled:after,mark.made:after,mark.out:after,mark.relay:after,mark.new:after,mark.accept:after,mark.reject:after,mark.processed:after,mark.collected:after {
