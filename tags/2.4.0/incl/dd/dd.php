...
class BkfDd{
    ...
    function bkf_dd_field_init () {
		...
		$sd_custom = array();
		foreach($co as $cos){
			if($cos['day'] == $today && $cos['cutoff'] <= date("G:i")){
				$sd_custom[] = $cos['method'];
			}
		}
		
		if(get_option('bkf_dd_setting')[$today] == 1 && strtotime(get_option('bkf_sd_setting')[$today]) <= strtotime(date("G:i"))){
			$sdcpassed = true;
		} else {
			$sdcpassed = false;
		}
    	...
    	<script id="bkf_ddfield">
		 ...
		 if (sdc.includes(currentShippingMethod) && d == "<?php echo date("j"); ?>" && y == "<?php echo date("Y"); ?>" && m == <?php echo date("n"); ?> - 1){
			 return [false,"sdc","<?php echo $sdctext; ?>"];
		 }
    ...
    function bkf_dd_checkout_validation( $fields, $errors ){
		...
		$notimeslottext = __('Please select a valid <strong>time slot</strong>.', 'bakkbone-florist-companion');
		
		if($_POST['delivery_date'] !== ''){
			$day = strtolower(date('l',strtotime($_POST['delivery_date'])));
			global $wpdb;
			$ts = array();
			$timeslots = $wpdb->get_results(
				"
		...
		if($_POST['delivery_date'] !== ''){
	        ...
	        } elseif(date("l, j F Y", strtotime($_POST['delivery_date'])) !== $_POST['delivery_date'] ){
				$errors->add( 'validation', $invalidtext );
			}
		} else {
            $errors->add( 'validation', $invalidtext );
