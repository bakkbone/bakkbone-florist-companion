										...
										<td><select class="select2 bkf-form-control product" name="product[p1][product]" required>
											...
											foreach($products as $product){
												if(! $product['has_child']){
													echo '<option value="'.addslashes($product['id']).'">'.addslashes($product['name']).' ('.addslashes($product['cat']).') — '.addslashes($product['price']).'</option>';
												}
											}
											?>
										</select>
			...
			jQuery(document).ready( function($) {
				...
				jQuery(addButton).click(function(){
					...
					jQuery('select.product').change(function(){
						id = jQuery(this).val();
						url = '<?php echo add_query_arg('action', 'bkf_product_value', admin_url('admin-ajax.php')); ?>' + '&id=' + id;
