...
class BKF_Ajax{
	...
	function __construct(){
		...
		$ajax = [
			...
			'dd_add_fee',
			'dd_remove_fee',
			'checkout_get_ajax_data',
			'retrieve_session_ts',
			'di',
			'dw',
			'product_value',
			'get_timeslots',
			'get_timeslots_for_order',
			'rates',
			'delivery_rates',
			'pickup_rates',
			'method_cost',
			...
			'check_weekday_fee',
			'check_ds_fee',
			'get_fee_for_timeslot',
			'phone_order',
			'sub_notice_dismissed',
			'resend_invoice',
			'valid_order',
			'search_suburbs',
			'search_suburbs_frontend',
			'dd',
			'nonfloral',
			'nonfloral_exclude_shipping',
			'order_search'
		];
		
		foreach ($ajax as $callback) {
			add_action('wp_ajax_bkf_'.$callback, [$this, $callback]);
		}
		...
		foreach ($nopriv as $callback) {
			add_action('wp_ajax_nopriv_bkf_'.$callback, [$this, $callback]);
		}
				
		// Order Notifier
		$features = get_option('bkf_features_setting');
		if($features['order_notifier']){
			add_action('wp_ajax_bkf_notifier', [$this, 'notifier']);
			add_action('wp_ajax_bkf_notifier_status', [$this, 'notifier_status']);
		}
						
		// Petals
		$bkfoptions = get_option("bkf_features_setting");
		if($bkfoptions["petals_on"]) {
			add_action('wp_ajax_petals_msg_frontend', [$this, 'petals_msg_frontend']);
			add_action('wp_ajax_petals_decision', [$this, 'petals_decision_ajax']);
			add_action('wp_ajax_petals_msg', [$this, 'petals_msg']);
			add_action('wp_ajax_petals_outbound', [$this, 'petals_outbound_ajax']); 
			add_action('wp_ajax_nopriv_petals_outbound', [$this, 'petals_outbound_ajax']);
			add_action('wp_ajax_bkf_cpp', [$this, 'cpp']);
		}
	...
	function checkout_get_ajax_data() {
		$array = [];
...
<notes>'.$msgbody.'</notes>
	...
	function search_suburbs(){
		...
		$string = stripslashes(sanitize_text_field($_REQUEST['query']));
	...
	function nonfloral() {
		...
		$nonfloral = $_REQUEST['nonfloral'] == 'true' ? true : false;
		$result = $nonfloral ? update_option('bkf_shipping_class_'.$term_id.'_nonfloral', true) : update_option('bkf_shipping_class_'.$term_id.'_nonfloral', false);
		
		if (bkf_debug()) {
			$debugnonfloral = $_REQUEST['nonfloral'] ? 'true' : 'false';
			bkf_debug_log(__CLASS__.'::'.__FUNCTION__.'(): '.$term_id.' - '.$debugnonfloral);
		}
	...
	function nonfloral_exclude_shipping() {
		$result = update_option('bkf_nonfloral_exclude_shipping', $_REQUEST['exclude']);
		echo json_encode($result);
		die();
	}
		...
		foreach ($orders as $id) {
			...
			if ($order instanceof WC_Order) {
				...
				if (is_array($shipping)) {
					$shipping = array_values($shipping)[0];
				}
				
				if ($shipping) {
					$pickup = $shipping->get_method_id() == 'local_pickup' ? true : false;
				} else {
					$pickup = false;
				}
				...
				$results[$id] = [
					'id'		=> $id,
					'dd'		=> $order->get_meta('_delivery_date', true) ? $order->get_meta('_delivery_date', true) : '',
					'dts'		=> $order->get_meta('_delivery_timestamp', true) ? $order->get_meta('_delivery_timestamp', true) : 0,
					'pickup'	=> $pickup,
					'physical'	=> $order->needs_shipping_address(),
					'delsuburb' => $order->get_shipping_city(),
					'recipient' => $order->get_shipping_first_name() . ' ' . $order->get_shipping_last_name(),
					'sender'	=> $order->get_billing_first_name() . ' ' . $order->get_billing_last_name(),
					'editlink'	=> $order->get_edit_order_url(),
					'wslink'	=> admin_url('admin-ajax.php?action=bkf_dw&order_id='.$id.'&nonce='.wp_create_nonce('bkf')),
