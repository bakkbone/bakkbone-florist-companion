...
class BKF_Email_Override{
	...
	function recovery_email_support_info($message){
		$message .= ' '.sprintf(__('If the stack trace below references FloristPress, you may wish to check the documentation at %s, or contact BAKKBONE Support at %s.','bakkbone-florist-companion'), 'https://plugins.bkbn.au/docs/bkf/', 'https://wordpress.org/support/plugin/bakkbone-florist-companion/');
	...
	function user_request_confirmed_email_content($email, $data){
		...
		__( 'Action Confirmed: %s' ), $email['description']
	);
		$mailer = WC()->mailer();
		$wrapped_message = $mailer->wrap_message($heading, $email);
		$wc_email = new WC_Email;
