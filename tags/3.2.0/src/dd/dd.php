...
class BKF_Delivery_Date_Core{
	...
	function __construct(){
		...
		add_action('woocommerce_checkout_update_order_meta', array($this, 'dd_order'), 10, 1);
		add_filter('woocommerce_email_order_meta_fields', array($this, 'dd_to_email'), 10, 3 );
		add_filter('woocommerce_order_details_before_order_table', array($this, 'dd_thankyou'), 10 , 1 );
		add_action('woocommerce_after_checkout_validation', array($this, 'dd_checkout_validation'), 10, 2 );
		add_action('wp_footer', array($this, 'dd_checkout_validation_js'));
		add_filter('manage_edit-shop_order_columns', array($this, 'dd_col_init'), 10, 1 );
		add_filter('manage_edit-shop_order_sortable_columns', array($this, 'dd_col_sort'), 10, 1 );
		add_action('pre_get_posts', array($this, 'dd_filter') );
		add_action('manage_shop_order_posts_custom_column', array($this, 'dd_col'), 10, 2 );
		add_action('woocommerce_process_shop_order_meta', array($this, 'dd_stamp'), PHP_INT_MAX, 2 );
		add_filter('woocommerce_checkout_required_field_notice', array($this, 'ship_deliver'),10,3);
		add_action('woocommerce_before_checkout_shipping_form', array($this, 'delivery_details'), 20);
	}
	...
	function dd_col( $column, $post_id ) {
	...
	function dd_stamp( $order_id, $order ) {
