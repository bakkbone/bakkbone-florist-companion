		...
		if( bkf_cart_has_physical() ) {
			...
			if($_POST['delivery_date'] !== ''){
				if( ! preg_match( '/[a-zA-Z]{6,9}\,\ \d{1,2}\ [a-zA-Z]{3,9}\ \d{4}/', $_POST['delivery_date'] ) ){
					...
					$availability = bkf_check_dd_availability($dateobject, WC()->session->get( 'chosen_shipping_methods' ));
					if(!$availability[0]){
						/* translators: %1s: Delivery date title. %2s: Error message. %3s: Delivery date title. */
						$failtext = sprintf(__('<strong>%1s Date<strong> error: %2s. Please select another %3s via the datepicker.', 'bakkbone-florist-companion'), $ddtitle, $availability[2], $ddtitle);
