...
function bkf_get_shipping_zones(){
	...
	foreach($zones as $zone){
		...
		foreach($methods as $method){
			...
			$cost = array_key_exists('cost', $instance) ? bkf_calc_cost($instance['cost']) : '0';
...
function bkf_get_shipping_rates_for_location($suburbraw, $postcode, $state, $country){
	$allrates = bkf_get_shipping_rates();
	$rates = [];
	
	$suburb = stripslashes($suburbraw);
...
function bkf_get_shipping_classes(){
	$termsquery = get_terms(['taxonomy' => 'product_shipping_class', 'hide_empty' => false]);
	$terms = [];
	
	foreach ($termsquery as $term) {
		$nonfloral = get_option('bkf_shipping_class_'.$term->term_id.'_nonfloral', false);
		$terms[] = [
...
function bkf_not_nonfloral(){
	$result = false;
	if (bkf_cart_has_physical()) {
		$nonfloralitems = [];
		$floralitems = [];
		foreach (WC()->cart->get_cart() as $item) {
			$class = $item['data']->get_shipping_class_id();
			$setting = get_option('bkf_shipping_class_'.$class.'_nonfloral', false);
			if ($setting) {
				$nonfloralitems[] = $item;
			} else {
				$floralitems[] = $item;
	...
	if (bkf_debug()) {
		...
		bkf_debug_log(__FUNCTION__.'(): '.$resultstring, 'debug');
	}
	return $result;
}

function bkf_fire_floristpress_methods(){
	if (get_option('bkf_nonfloral_exclude_shipping', false)) {
		return bkf_not_nonfloral();
	} else {
		return true;
	}
}
