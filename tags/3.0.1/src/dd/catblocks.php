				...
				?><div class="inside bkf-inside">
					...
					<?php foreach($cb as $thiscblock){
							if($thiscblock['category'] == $catarray['term_id']){
									echo '<p>'.$thiscblock['date'].' <em><a href="'.admin_url('admin-ajax.php?action=bkf_cb_del&nonce='.$nonce.'&id='.$thiscblock['id']).'">'.esc_html__('Delete','bakkbone-florist-companion').'</a></em>';
									echo '</p>';
							} else {}
						}
						?></div><?php
