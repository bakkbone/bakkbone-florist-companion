...
add_action('woocommerce_checkout_update_order_review', 'checkout_update_refresh_shipping_methods', 999, 1);

function BKF_Suburbs_filter_shipping_methods($rates, $package) {
	
	if (bkf_fire_floristpress_methods()) {
		if (bkf_debug()) {
		...
		foreach ($rates as $rate_key => $rate) {
			if ($rate->get_method_id() == 'floristpress') {
				unset($rates[$rate_key]);
			}
		}
	}
