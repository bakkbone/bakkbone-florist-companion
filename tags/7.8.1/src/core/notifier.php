...
class BKF_Notifier{
	...
	function __construct(){
		...
		add_action("plugins_loaded", [$this, "bkf_update_notifier_check"]);
	...
	function bkf_notifier_js() {
	    $audiourl = __BKF_URL__ . 'assets/audio/';
		...
		<script type="text/javascript" id="order_notifier_js">
			...
			function bkfToggleNotifierStatus(){
				...
				if (toggle.checked == true){
					<?php if (bkf_debug(true)) { ?>console.info("<?php esc_html_e('Notifier enabled', 'bakkbone-florist-companion'); ?>");<?php } ?>
					notifierInterval = setInterval(runNotifierCheck, 30000);
					var toggleUrl = "<?php echo admin_url('admin-ajax.php?action=bkf_notifier_status&status=1&user='.get_current_user_id()); ?>";
					jQuery( function($){ jQuery.ajax({url: toggleUrl, type: 'POST'}); });
				} else {
					<?php if (bkf_debug(true)) { ?>console.info("<?php esc_html_e('Notifier disabled', 'bakkbone-florist-companion'); ?>");<?php } ?>
					if(typeof notifierInterval !== 'undefined'){
						clearInterval(notifierInterval);
						var toggleUrl = "<?php echo admin_url('admin-ajax.php?action=bkf_notifier_status&status=0&user='.get_current_user_id()); ?>";
						jQuery( function($){ jQuery.ajax({url: toggleUrl, type: 'POST'}); });
					}
				}
			}
			...
			function runNotifierCheck(){
				jQuery( function($){
					...
					var url = "<?php echo admin_url('admin-ajax.php?action=bkf_notifier&timestamp='); ?>" + ts;
					jQuery.ajax({url: url, type: 'POST', success: function(result){
						...
						if(JSON.parse(result).length !== 0){
							...
							audio.loop = false;
							audio.play();
						<?php if (bkf_debug(true)) { ?>} else {
							console.log("<?php esc_html_e('No orders found between the following times:', 'bakkbone-florist-companion'); ?>" + ' ' + tsHours + ':' + tsMinutes.substr(-2) + ':' + tsSeconds.substr(-2) + ', ' + currentTimeHours + ':' + currentTimeMinutes.substr(-2) + ':' + currentTimeSeconds.substr(-2) );<?php } ?>
						}
					}});
