...
class BKF_Delivery_Date_Calendar{
	...
	function __construct(){
		add_action("admin_menu", [$this, "page"], 3);
	}
	...
	<script>
	  document.addEventListener('DOMContentLoaded', function() {
		...
		$orders = wc_get_orders(array(
			'status'   => array('wc-new','wc-accept','wc-processing','wc-completed','wc-scheduled','wc-prepared','wc-collect','wc-out','wc-relayed','wc-invoiced','wc-processed','wc-collected'),
			'limit'   => '-1',
		...
		foreach($orders as $order){
			...
			if($order->get_meta('_delivery_timestamp',true) !== ''){
				...
				$ws = admin_url('admin-ajax.php?action=bkf_dw&order_id='.$id.'&nonce='.wp_create_nonce('bkf'));
				$date = wp_date("Y-m-d",$order->get_meta('_delivery_timestamp',true));
				$recipient = str_replace('"', '\"', $order->get_shipping_first_name().' '.$order->get_shipping_last_name());
				$customer = str_replace('"', '\"', $order->get_billing_first_name().' '.$order->get_billing_last_name());
				$suburb = str_replace('"', '\"', $order->get_shipping_city());
				if($order->needs_shipping_address()){
					...
					if($order->get_meta('_delivery_timeslot_id',true) !== ''){
						...
						if($ts){
							$start = $ts['start'];
							$end = $ts['end'];
							echo str_replace(["\r","\n"],"<br/>",'{ title: "#'.$order->get_id().' - '. $recipient . ', ' . $suburb . '", url: "'.html_entity_decode($url).'", start: "'.$date.' '.$start.':00", end: "'.$date.' '.$end.':00", classNames: ["shipped"], wsurl: "'.$ws.'", content: "'.bkf_get_timeslot_string($order->get_meta('_delivery_timeslot_id',true)).'<br/>'.$address.'" }, ');
						} else {
							...
							echo str_replace(["\r","\n"],"<br/>",'{ title: "#'.$order->get_id().' - '. $recipient . ', ' . $suburb . '", url: "'.html_entity_decode($url).'", start: "'.$date.' '.$start.':00", end: "'.$date.' '.$end.':00", classNames: ["shipped"], wsurl: "'.$ws.'", content: "'.$timeslot.'<br/>'.$address.'" }, ');
						}
						...
						echo str_replace(["\r","\n"],"<br/>",'{ title: "#'.$order->get_id().' - '. $recipient . ', ' . $suburb . '", url: "'.html_entity_decode($url).'", start: "'.$date.'", classNames: ["shipped"], wsurl: "'.$ws.'", content: "'.$address.'" }, ');
					}
				} else {
					if($order->get_meta('_delivery_timeslot_id',true) !== ''){
						$allts = bkf_get_timeslots_associative();
						...
						if($ts){
							$start = $ts['start'];
							$end = $ts['end'];
							echo str_replace(["\r","\n"],"<br/>",'{ title: "#'.$order->get_id().' - '.esc_html__('Pickup', 'bakkbone-florist-companion').', '.$customer.'", url: "'.html_entity_decode($url).'", start: "'.$date.' '.$start.':00", end: "'.$date.' '.$end.':00", classNames: ["unshipped"], wsurl: "'.$ws.'", content: "'.bkf_get_timeslot_string($order->get_meta('_delivery_timeslot_id',true)).'" }, ');
						} else {
							$timeslot = $order->get_meta('_delivery_timeslot',true);
							$array = explode(" - ",$timeslot);
							$start = date("H:i",strtotime($array[0]));
							$end = date("H:i",strtotime($array[1]));
							echo str_replace(["\r","\n"],"<br/>",'{ title: "#'.$order->get_id().' - '.esc_html__('Pickup', 'bakkbone-florist-companion').', '.$customer.'", url: "'.html_entity_decode($url).'", start: "'.$date.' '.$start.':00", end: "'.$date.' '.$end.':00", classNames: ["unshipped"], wsurl: "'.$ws.'", content: "'.$timeslot.'" }, ');
						}
					} elseif($order->get_meta('_delivery_timeslot',true) !== '') {
						$timeslot = $order->get_meta('_delivery_timeslot',true);
						$array = explode(" - ",$timeslot);
						...
						echo str_replace(["\r","\n"],"<br/>",'{ title: "#'.$order->get_id().' - '.esc_html__('Pickup', 'bakkbone-florist-companion').', '.$customer.'", url: "'.html_entity_decode($url).'", start: "'.$date.'", classNames: ["unshipped"], wsurl: "'.$ws.'", content: "'.$order->get_meta('_delivery_date',true).'" }, ');
					}
				}
