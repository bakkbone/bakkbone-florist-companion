...
class BKF_Core{
	...
	function bkf_notes_email( $fields, $sent_to_admin, $order ) {
		$fields['_shipping_notes'] = array(
			...
			'value' => $order->get_meta( '_shipping_notes', true ),
		);
		$fields['_card_message'] = array(
			'label' => __('Card Message', 'bakkbone-florist-companion'),
			'value' => $order->get_meta( '_card_message', true ),
		);
		return $fields;
	}
	
	function bkf_ot_thankyou ( $order ) {
		$cm = $order->get_meta( '_card_message', true );
	...
	function bkf_cd_thankyou ( $order ) {
		$sn = $order->get_meta( '_shipping_notes', true );
	...
	function bkf_checkout_field_update_order_meta( $order_id ) {
		...
		if ( ! empty( $_POST['shipping_notes'] ) ) {
			$order->update_meta_data( '_shipping_notes', sanitize_text_field( $_POST['shipping_notes'] ) );
		}
		if ( ! empty( $_POST['card_message'] ) ) {
			$order->update_meta_data( '_card_message', sanitize_text_field( $_POST['card_message'] ) );
		}
		$order->save();
	}
	...
	public function bkf_cm_metabox_callback( $post_or_order_object ){
		...
		$cardmessage = $order->get_meta( '_card_message', true );
