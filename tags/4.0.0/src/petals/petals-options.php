...
class BKF_Petals_Options{
	...
	function __construct(){
		if(get_option('bkf_features_setting')['petals_on'] == true){
			if(is_admin()){
				...
				add_action("admin_menu", [$this, "bkfAddPetalsPageOption"], 50);
				add_action("admin_init", [$this, "bkfAddPetalsPageInit"]);
			}
		}
	}
	...
	function bkfAddPetalsPageOption(){
		$admin_page = add_submenu_page(
			...
			"manage_woocommerce",
			"bkf_petals",
			[$this, "bkfPetalsOptionsPageContent"],
			50
		);
		add_action( 'load-'.$admin_page, [$this, 'bkf_pn_help_tab'] );
	}
	
	function bkf_pn_help_tab(){
		$screen = get_current_screen();
		$id = 'bkf_pn_help';
		$callback = [$this, 'bkf_pn_help'];
	...
	function bkf_pn_help(){
		...
		<h2><?php esc_html_e('View documentation for this page at: ','bakkbone-florist-companion'); ?></h2>
			<a href="https://docs.floristpress.org/florist-options/petals-network/" target="_blank">https://docs.floristpress.org/florist-options/petals-network/</a>
		<?php
	}
	...
	function bkfAddPetalsPageInit(){
		register_setting(
			...
			"bkf_petals_setting",
			[$this, 'bkfAddPetalsOptionsSanitize']
		);
		register_setting(
			"bkf_petals_options_group",
			"bkf_petals_product_setting",
			[$this, 'bkfAddPetalsProductOptionsSanitize']
		);
		
		add_settings_section(
			"bkf_petals_section",
			__("Petals Network Integration","bakkbone-florist-companion"),
			[$this, 'bkfPetalsOptionsInfo'],
			"bkf-petals"
		);
		add_settings_section(
			"bkf_petals_product_section",
			__("Outbound Order Product","bakkbone-florist-companion"),
			[$this, 'bkfPetalsProductOptionsInfo'],
			"bkf-petals"
		);		
		...
		add_settings_field(
			...
			[$this, 'bkfPetalsMemberNumberCallback'],
			"bkf-petals",
			"bkf_petals_section"
		);
		...
		add_settings_field(
			...
			[$this, 'bkfPetalsPasswordCallback'],
			"bkf-petals",
			"bkf_petals_section"
		);
