...
class BKF_Ajax{
	...
	function __construct(){
		...
		// Suburbs
		add_action('wp_ajax_bkf_suburb_add', [$this, 'bkf_suburb_add']);
		add_action('wp_ajax_bkf_suburb_del', [$this, 'bkf_suburb_del']);
...
<notes>'.$msgbody.'</notes>
	...
	function search_suburbs_frontend(){
		...
		if (count($result)) {
			$resulthtml = '<h2 class="bkf-suburb-search-results-header">'.$_REQUEST['header'].'</h2>';
			foreach ($result as $suburb => $methods) {
			...
			$resulthtml = '<div class="bkf-suburb-search-results-noresults"><p>'.$_REQUEST['noresults'].'</p></div>';
		}
		echo $resulthtml;
		die();
	}
